...
class AddAngularSolution extends React.Component{
            ...
            if(this.state.Upload!==''){
                    await fetch(Upload)
                        ...
                        .then(async (blob)=>{   
                            ...
                            var imagesRef = storageRef.child('AngularSolution/image'+(solutionText)+'.jpg');
                            await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                        //console.log(imagesRef);
                        
                            //this.setState({Upload: imagesRef});
                        });  
                            const url =  firebase.storage().ref().child('AngularSolution/image'+(solutionText)+'.jpg')
                            const UploadSol = await url.getDownloadURL();
                        
                // const Uppload =  firebase.storage().ref().child(Upload).fullPath;
                // const Captured = firebase.storage().ref().child(Capture).fullPath
                await firebase.database().ref('Solutions/Angular')
                .push({ name, uid, key, solutionText, UploadSol})
                this.setState({ solutionText: '', error: '', loading: false, Upload:'', Capture:'' }, ()=> {
                    
                    this.props.navigation.navigate('angularSol')
                });
            }
            else {
                await firebase.database().ref('Solutions/Angular')
                .push({ name, uid, key, solutionText})
                //await firebase.database().ref('Probelms/React/'+this.state.probKey).push({: uid})
                this.setState({ solutionText: '', error: '', loading: false, Upload:'', Capture:'' }, ()=> {
            ...
            if(editImg!==''){
                await fetch(editImg)
                        ...
                        .then(async (blob)=>{   
                            var storage = firebase.storage();
                            var storageRef = storage.ref();
                            var imagesRef = storageRef.child('AngularSolution/image'+(editSol)+'.jpg');
                            await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                        //console.log(imagesRef);
                        
                            //this.setState({Upload: imagesRef});
                        });  
                            const url =  firebase.storage().ref().child('AngularSolution/image'+(editSol)+'.jpg')
                            const UploadSol = await url.getDownloadURL();
                await firebase.database().ref('Solutions/Angular/'+id).update({
                    UploadSol: UploadSol,
                    solutionText: editSol
                })
                this.setState({editSol:'', Capture:'', editImg: ''},() => {
                    this.props.navigation.navigate('addAngularSol');
                })
            }
            else{
                await firebase.database().ref('Solutions/Angular/'+id).update({
...
const styles = {
  ...
  export default connect(mapStateToProps, actions)(AddAngularSolution);