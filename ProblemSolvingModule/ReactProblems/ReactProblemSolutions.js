...
class Solutions extends React.Component{
    ...
    constructor(props){
        ...
        this.state = {
            ...
            states: true,
            solution:[],
            noImg: this.props.navigation.state.params.noImg
        }
    }
    ...
    componentDidMount() {
        ...
        // this.state.solution = [];
            ...
            if(done===null){
                firebase.database().ref('/Solutions/React')
                .on('value', snapshot => {
    ...
    addReactSolutionHandler() {
        const tId = this.state.tId;
          solution = [];
        const probKey = this.state.solution.map(i=>i.probKey)
        console.log(probKey)
        //  this.state.solution=[];
         //this.isMount=false;
         this.props.navigation.navigate('addreactsol',{key:tId});
    }
    ...
    handleEdit = () => {
        ...
        this.props.navigation.navigate('addreactprob', {des:des, exp:exp, img:img, key:tId, states:states});
    }
    ...
    handleDelete = async () => {
        ...
        await firebase.database().ref('Probelms/React/'+this.state.tId).remove();
        this.props.navigation.navigate('reactProb');
            firebase.database().ref('/Solutions/React')
            .on('value', snapshot => {
            snapshot.forEach((child)=>{
                if(child.val().key===this.state.tId){
                    firebase.database().ref('/Solutions/React/' + child.key).remove();
                }
            });
        })
    }
    ...
    handleEditSolution = (sol, upload, id) => {
        ...
        this.props.navigation.navigate('addreactsol', {editSol: sol, editImg: Upload, id: id, states: this.state.states})
    }

    handleDeleteSolution = async (id) => {
         loading=true;
         solution=[];
        await firebase.database().ref('Solutions/React/'+id).remove();
