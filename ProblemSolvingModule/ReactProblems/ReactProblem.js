...
class ReactProblems extends React.Component{
    state = {
        random: null,
        isMounted: false,
        count:0,
    ...
    componentDidMount(){
        ...
        if(done===null){
            const res = firebase.database().ref('/Probelms/React')
            .on('value', snapshot => {
    ...
    addReactProblemHandler(){
         problems = [];
        this.props.navigation.navigate('addreactprob',{des: this.state.des, exp: this.state.exp, img: this.state.img, states: this.state.states, key: this.state.tId});
    }

    reactProblemSolutionHandler(description, explanation, name, image, language, tId, image1, uid){
        problems=[];
         console.log(tId)
        this.props.navigation.navigate('reactSolution',{description: description, explanation: explanation, name: name, image: image, language: language, tId: tId, image1: image1, uid});
            ...
            return(
                <View style={{ paddingTop: Platform.OS === 'android' ? StatusBar.currentHeight : 0, flex: 1 }}>
                     ...
                     <ScrollView alwaysBounce>
                        ...
                        <View style={{width:screenWidth*.5, height:screenHeight*.15, marginTop:'10%',marginLeft:'25%',marginRight:5,color:'black'}}>
