...
class AddCProblem extends React.Component{
    state = {
        isFocused: false, 
            ...
            if(this.state.descriptionText===''){
                ...
                if(this.state.Upload!==''){
                    ...
                    .then(async (blob)=>{   
                        ...
                        var imagesRef = storageRef.child('CProblems/image'+(descriptionText)+'.jpg');
                        await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                    //console.log(imagesRef);
                    
                        //this.setState({Upload: imagesRef});
                    });  
                        const url =  firebase.storage().ref().child('CProblems/image'+(descriptionText)+'.jpg')
                        const UploadProb = await url.getDownloadURL();
                        await this.props.CproblemStored(name, uid, Langauge, descriptionText, explanationText, UploadProb)
                        this.setState({descriptionText:'', explanationText:'', Upload: '', loading: false},() => {
                            this.props.navigation.navigate('cProb');
                        })
                }
                else {
                    await this.props.CproblemStored1(name, uid, Langauge, descriptionText, explanationText)
        ...
        if(des===''){
            ...
            if(this.state.img!==''){
                ...
                .then(async (blob)=>{   
                    var storage = firebase.storage();
                    var storageRef = storage.ref();
                    var imagesRef = storageRef.child('CProblems/image'+(des)+'.jpg');
                    await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                });  
                    var storageRef = firebase.storage().ref();
                    const url =  firebase.storage().ref().child('CProblems/image'+(des)+'.jpg')
                    const UploadProb = await url.getDownloadURL();
                await firebase.database().ref('Probelms/C/'+this.state.key).update({
                    Upload: img,
                    descriptionText: des,
                    explanationText: exp,
                })
...
const styles = {
  ...
  export default connect(mapStateToProps, actions)(AddCProblem);