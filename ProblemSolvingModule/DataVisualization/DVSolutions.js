...
class DVSolutions extends React.Component{
    ...
    componentDidMount() {
        ...
        // this.state.solution = [];
            ...
            if(done===null){
                firebase.database().ref('/Solutions/DV')
                .on('value', snapshot => {
    ...
    addReactSolutionHandler() {
        ...
        //  this.state.solution=[];
         //this.isMount=false;
         this.props.navigation.navigate('addDVSolution',{key:tId});
    }
    ...
    handleEdit = () => {
        ...
        this.props.navigation.navigate('addDVProb', {des:des, exp:exp, img:img, key:tId, states:states});
    }
    ...
    handleDelete = async () => {
        ...
        await firebase.database().ref('Probelms/DV/'+this.state.tId).remove();
        this.props.navigation.navigate('dvProb');
            firebase.database().ref('/Solutions/DV')
            .on('value', snapshot => {
            snapshot.forEach((child)=>{
                if(child.val().key===this.state.tId){
                    firebase.database().ref('/Solutions/DV/' + child.key).remove();
                }
            });
        })
    }
    ...
    handleEditSolution = (sol, upload, id) => {
        ...
        this.props.navigation.navigate('addDVSolution', {editSol: sol, editImg: Upload, id: id, states: this.state.states})
    }
    ...
    handleDeleteSolution = async (id) => {
         loading=true;
         solution=[];
        await firebase.database().ref('Solutions/DV/'+id).remove();
