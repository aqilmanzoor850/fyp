...
class AddAssemblySolution extends React.Component{
            ...
            if(this.state.Upload!==''){
                    ...
                    await fetch(Upload)
                        ...
                        .then(async (blob)=>{   
                            ...
                            var imagesRef = storageRef.child('AssemblySolutions/image'+(solutionText)+'.jpg');
                            await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                        //console.log(imagesRef);
                        
                            //this.setState({Upload: imagesRef});
                        });  
                            const url =  firebase.storage().ref().child('AssemblySolutions/image'+(solutionText)+'.jpg')
                ...
                await firebase.database().ref('Solutions/Assembly')
                ...
                this.setState({ solutionText: '', error: '', loading: false, Upload:'', Capture:'' }, ()=> {
                    ...
                    this.props.navigation.navigate('assemblySolution')
                });
            }
            else {
                await firebase.database().ref('Solutions/Assembly')
    ...
    onUpdatePress = async () => {
        ...
        if(editSol===''){
            ...
            this.setState({error: ''});
            if(editImg!==''){
                console.log('idhr tu a rha a na')
                await fetch(editImg)
                .then((response)=>response.blob())
                .then(async (blob)=>{   
                    var storage = firebase.storage();
                    var storageRef = storage.ref();
                    var imagesRef = storageRef.child('AssemblySolutions/image'+(editSol)+'.jpg');
                    await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                //console.log(imagesRef);
                
                    //this.setState({Upload: imagesRef});
                });  
                    const url =  firebase.storage().ref().child('AssemblySolutions/image'+(editSol)+'.jpg')
                    const Uploadimg = await url.getDownloadURL();
                await firebase.database().ref('Solutions/Assembly/'+id).update({
                    UploadSol: Uploadimg,
...
const styles = {
  ...
  export default connect(mapStateToProps, actions)(AddAssemblySolution);