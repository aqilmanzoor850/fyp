...
class AssemblySolutions extends React.Component{
    ...
    componentDidMount() {
        ...
        // this.state.solution = [];
            ...
            if(done===null){
                firebase.database().ref('/Solutions/Assembly')
                .on('value', snapshot => {
    ...
    addReactSolutionHandler() {
        ...
        //  this.state.solution=[];
         //this.isMount=false;
         this.props.navigation.navigate('addAssemblySolution',{key:tId});
    }
    ...
    handleEdit = () => {
        ...
        this.props.navigation.navigate('addAssemblyProb', {des:des, exp:exp, img:img, key:tId, states:states});
    }
    ...
    handleDelete = async () => {
        ...
        await firebase.database().ref('Probelms/Assembly/'+this.state.tId).remove();
        this.props.navigation.navigate('assemblyProb');
            firebase.database().ref('/Solutions/Assembly')
            .on('value', snapshot => {
            snapshot.forEach((child)=>{
                if(child.val().key===this.state.tId){
                    firebase.database().ref('/Solutions/Assembly/' + child.key).remove();
                }
            });
        })
    }
    ...
    handleEditSolution = (sol, upload, id) => {
        ...
        this.props.navigation.navigate('addAssemblySolution', {editSol: sol, editImg: Upload, id: id, states: this.state.states})
    }
    ...
    handleDeleteSolution = async (id) => {
         loading=true;
         solution=[];
        await firebase.database().ref('Solutions/Assembly/'+id).remove();
                        ...
                        <View style={{width:screenWidth, borderBottomWidth:2, borderBottomColor:'grey'}}></View>
                            ...
                            <ScrollView alwaysBounceVertical style={{display:'flex', width:screenWidth, height:screenHeight*.85, backgroundColor:'white'}}>
                                ...
                                <FlatList
                                    ...
                                    renderItem={({item}) => <SolutionItem userId={item.userId} solution={item.solutions} upload={item.upload} capture={item.capture} uid={this.state.uid} id={item.id} handleSolutionImage={this.handleSolutionImage} size={this.state.size} handleEditDeleteAlert={this.handleEditDeleteAlertSolution} keyss={this.state.tId} probKey={item.probKey} />}
