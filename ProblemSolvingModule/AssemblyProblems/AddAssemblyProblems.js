...
class AddAssemblyProblem extends React.Component{
    state = {
        isFocused: false, 
        ...
        if(this.state.descriptionText===''){
            ...
            if(this.state.Upload!==''){
                ...
                .then(async (blob)=>{   
                    ...
                    var imagesRef = storageRef.child('AssemblyProblems/image'+(descriptionText)+'.jpg');
                    await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                //console.log(imagesRef);
                
                    //this.setState({Upload: imagesRef});
                });  
                    const url =  firebase.storage().ref().child('AssemblyProblems/image'+(descriptionText)+'.jpg')
                    const UploadProb = await url.getDownloadURL();
                    await this.props.AssemblyproblemStored(name, uid, Langauge, descriptionText, explanationText, UploadProb)
                    this.setState({descriptionText:'', explanationText:'', Upload: '', loading: false},() => {
                        this.props.navigation.navigate('assemblyProb');
                    })
            }
            else {
                await this.props.AssemblyproblemStored1(name, uid, Langauge, descriptionText, explanationText)
                this.setState({descriptionText:'', explanationText:'', loading: false},() => {
                    this.props.navigation.navigate('assemblyProb');
                })
            }
        ...
        if(des===''){
            ...
            if(this.state.img!==''){
                ...
                .then(async (blob)=>{   
                    var storage = firebase.storage();
                    var storageRef = storage.ref();
                    var imagesRef = storageRef.child('AssemblyProblems/image'+(des)+'.jpg');
                    await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                });  
                    var storageRef = firebase.storage().ref();
                    const url =  firebase.storage().ref().child('AssemblyProblems/image'+(des)+'.jpg')
                    const UploadProb = await url.getDownloadURL();
                await firebase.database().ref('Probelms/Assembly/'+this.state.key).update({
                    Upload: img,
                    descriptionText: des,
                    explanationText: exp,
                })
...
const styles = {
  ...
  export default connect(mapStateToProps, actions)(AddAssemblyProblem);