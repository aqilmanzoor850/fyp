...
class PythonSolutions extends React.Component{
    ...
    componentDidMount() {
        ...
        // this.state.solution = [];
            ...
            if(done===null){
                firebase.database().ref('/Solutions/Python')
                .on('value', snapshot => {
    ...
    addReactSolutionHandler() {
        ...
        //  this.state.solution=[];
         //this.isMount=false;
         this.props.navigation.navigate('addPythonSolution',{key:tId});
    }
    ...
    handleEdit = () => {
        ...
        this.props.navigation.navigate('addPythonProb', {des:des, exp:exp, img:img, key:tId, states:states});
    }
    ...
    handleDelete = async () => {
        ...
        await firebase.database().ref('Probelms/Python/'+this.state.tId).remove();
        this.props.navigation.navigate('pythonProb');
            firebase.database().ref('/Solutions/Python')
            .on('value', snapshot => {
            snapshot.forEach((child)=>{
                if(child.val().key===this.state.tId){
                    firebase.database().ref('/Solutions/Python/' + child.key).remove();
                }
            });
        })
    }
    ...
    handleEditSolution = (sol, upload, id) => {
        ...
        this.props.navigation.navigate('addPythonSolution', {editSol: sol, editImg: Upload, id: id, states: this.state.states})
    }
    ...
    handleDeleteSolution = async (id) => {
         loading=true;
         solution=[];
        await firebase.database().ref('Solutions/Python/'+id).remove();
...
export default PythonSolutions;