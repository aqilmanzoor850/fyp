...
class AddPythonSolution extends React.Component{
            ...
            if(this.state.Upload!==''){
                    ...
                    await fetch(Upload)
                        ...
                        .then(async (blob)=>{   
                            ...
                            var imagesRef = storageRef.child('PythonSolutions/image'+(solutionText)+'.jpg');
                            await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                        });  
                            const url =  firebase.storage().ref().child('PythonSolutions/image'+(solutionText)+'.jpg')
                            const UploadSol = await url.getDownloadURL();
                await firebase.database().ref('Solutions/Python')
                .push({ name, uid, key, solutionText, UploadSol})
                this.setState({ solutionText: '', error: '', loading: false, Upload:'', Capture:'' }, ()=> {
                    
                    this.props.navigation.navigate('pythonSolution')
                });
            }
            else {
                await firebase.database().ref('Solutions/Python')
        ...
        if(editSol===''){
            ...
            if(editImg!==''){
                ...
                .then(async (blob)=>{   
                    var storage = firebase.storage();
                    var storageRef = storage.ref();
                    var imagesRef = storageRef.child('PythonSolutions/image'+(editSol)+'.jpg');
                    await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                });  
                    const url =  firebase.storage().ref().child('PythonSolutions/image'+(editSol)+'.jpg')
                    const Uploadimg = await url.getDownloadURL();
                await firebase.database().ref('Solutions/Python/'+id).update({
                    UploadSol: Uploadimg,
...
const styles = {
  ...
  export default connect(mapStateToProps, actions)(AddPythonSolution);