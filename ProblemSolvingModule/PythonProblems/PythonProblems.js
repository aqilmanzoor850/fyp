...
class PythonProblems extends React.Component{
    state = {
        random: null,
        isMounted: false,
        count:0,
    ...
    componentDidMount(){
        ...
        if(done===null){
            const res = firebase.database().ref('/Probelms/Python')
            .on('value', snapshot => {
    ...
    addReactProblemHandler(){
         problems = [];
        this.props.navigation.navigate('addPythonProb',{des: this.state.des, exp: this.state.exp, img: this.state.img, states: this.state.states, key: this.state.tId});
    }

    reactProblemSolutionHandler(description, explanation, name, image, language, tId, image1, uid){
        problems=[];
        // console.log(tId)
        this.props.navigation.navigate('pythonSolution',{description: description, explanation: explanation, name: name, image: image, language: language, tId: tId, image1: image1, uid});
