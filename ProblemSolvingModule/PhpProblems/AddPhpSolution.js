...
class AddPhpSolution extends React.Component{
            ...
            if(this.state.Upload!==''){
                    ...
                    await fetch(Upload)
                        ...
                        .then(async (blob)=>{   
                            ...
                            var imagesRef = storageRef.child('PhpSolutions/image'+(solutionText)+'.jpg');
                            await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                        });  
                            const url =  firebase.storage().ref().child('PhpSolutions/image'+(solutionText)+'.jpg')
                            const UploadSol = await url.getDownloadURL();
                await firebase.database().ref('Solutions/Php')
                .push({ name, uid, key, solutionText, UploadSol})
                this.setState({ solutionText: '', error: '', loading: false, Upload:'', Capture:'' }, ()=> {
                    
                    this.props.navigation.navigate('phpSolution')
                });
            }
            else {
                await firebase.database().ref('Solutions/Php')
        ...
        if(editSol===''){
            ...
            if(editImg!==''){
                ...
                .then(async (blob)=>{   
                    var storage = firebase.storage();
                    var storageRef = storage.ref();
                    var imagesRef = storageRef.child('PhpSolutions/image'+(editSol)+'.jpg');
                    await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                });  
                    const url =  firebase.storage().ref().child('PhpSolutions/image'+(editSol)+'.jpg')
                    const Uploadimg = await url.getDownloadURL();
                await firebase.database().ref('Solutions/Php/'+id).update({
                    UploadSol: Uploadimg,
...
const styles = {
  ...
  export default connect(mapStateToProps, actions)(AddPhpSolution);