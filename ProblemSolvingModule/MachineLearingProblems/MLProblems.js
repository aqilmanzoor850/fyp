...
class Item extends React.PureComponent {
    ...
    render(){
        ...
        return (
            <View>
                <TouchableOpacity style={[(firebase.auth().currentUser.uid===this.props.uid) ? styles.key1 : styles.key2]} 
                onPress={()=>this.props.reactProblemSolutionHandler(this.props.description, this.props.explanation, 
                this.props.name, this.props.image, this.props.language, this.props.tId, this.props.image1, this.props.uid)}>
                       
                 <Text adjustsFontSizeToFit={true}
                         style={{ justifyContent:'center', alignContent:'center', marginLeft: 10, fontSize:0.05*screenWidth}}>
...
class MLProblems extends React.Component{
    state = {
        random: null,
        isMounted: false,
        count:0,
    ...
    componentDidMount(){
        ...
        if(done===null){
            const res = firebase.database().ref('/Probelms/ML')
            .on('value', snapshot => {
    ...
    addReactProblemHandler(){
         problems = [];
        this.props.navigation.navigate('addMLProb',{des: this.state.des, exp: this.state.exp, img: this.state.img, states: this.state.states, key: this.state.tId});
    }

    reactProblemSolutionHandler(description, explanation, name, image, language, tId, image1, uid){
        problems=[];
        // console.log(tId)
        this.props.navigation.navigate('mlSolution',{description: description, explanation: explanation, name: name, image: image, language: language, tId: tId, image1: image1, uid});
                     ...
                     <LinearGradient colors={['#48D1CC', '#eee']} style={{ width: screenWidth, height:screenHeight }}>
                        ...
                        <View style={{display: 'flex', justifyContent:'center',marginTop:'5%',paddingBottom:'15%',paddingTop:'-7%',paddingRight:'5%',
                            ...
                            borderTopRightRadius: 30,
                                ...
                                <Text style={{fontWeight:'900',marginTop:'10%', marginLeft:'5%',fontSize:30,
