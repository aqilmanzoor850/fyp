...
class MLSolutions extends React.Component{
    ...
    componentDidMount() {
        ...
        // this.state.solution = [];
            ...
            if(done===null){
                firebase.database().ref('/Solutions/ML')
                .on('value', snapshot => {
    ...
    addReactSolutionHandler() {
        ...
        //  this.state.solution=[];
         //this.isMount=false;
         this.props.navigation.navigate('addMLSolution',{key:tId});
    }
    ...
    handleEdit = () => {
        ...
        this.props.navigation.navigate('addMLProb', {des:des, exp:exp, img:img, key:tId, states:states});
    }
    ...
    handleDelete = async () => {
        ...
        await firebase.database().ref('ProbelmsML/'+this.state.tId).remove();
        this.props.navigation.navigate('mlProb');
            firebase.database().ref('/Solutions/ML')
            .on('value', snapshot => {
            snapshot.forEach((child)=>{
                if(child.val().key===this.state.tId){
                    firebase.database().ref('/Solutions/ML/' + child.key).remove();
                }
            });
        })
    }
    ...
    handleEditSolution = (sol, upload, id) => {
        solution=[];
        this.props.navigation.navigate('addMLSolution', {editSol: sol, editImg: upload, id: id, states: this.state.states})
    }

    handleDeleteSolution = async (id) => {
         loading=true;
         solution=[];
        await firebase.database().ref('Solutions/ML/'+id).remove();
                    ...
                    <LinearGradient colors={['#48D1CC', '#eee']} style={{ width: screenWidth, height:screenHeight }}>
                        ...
                        <View style={{width:screenWidth, borderBottomWidth:2, borderBottomColor:'grey'}}></View>
                            ...
                            <ScrollView alwaysBounceVertical style={{display:'flex', width:screenWidth, height:screenHeight*.85, backgroundColor:'white'}}>
                                ...
                                <TouchableOpacity onPress={this.handleImageSize}>
                                    <Image 
