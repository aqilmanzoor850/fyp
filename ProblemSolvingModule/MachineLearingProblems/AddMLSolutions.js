...
class AddMLSolution extends React.Component{
            ...
            this.setState({ error: '' });
                ...
                await fetch(Upload)
                    ...
                    .then(async (blob)=>{   
                        ...
                        var imagesRef = storageRef.child('MLSolutions/image'+(solutionText)+'.jpg');
                        await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                    //console.log(imagesRef);
                    
                        //this.setState({Upload: imagesRef});
                    });  
                        const url =  firebase.storage().ref().child('MLSolutions/image'+(solutionText)+'.jpg')
        ...
        if(editSol===''){
            ...
            await fetch(editImg)
                    ...
                    .then(async (blob)=>{   
                        var storage = firebase.storage();
                        var storageRef = storage.ref();
                        var imagesRef = storageRef.child('MLSolutions/image'+(editSol)+'.jpg');
                        await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                    //console.log(imagesRef);
                    
                        //this.setState({Upload: imagesRef});
                    });  
                        const url =  firebase.storage().ref().child('MLSolutions/image'+(editSol)+'.jpg')
                        const UploadSol = await url.getDownloadURL();
            await firebase.database().ref('Solutions/ML/'+id).update({
                Upload: editImg,
...
const styles = {
  ...
  export default connect(mapStateToProps, actions)(AddMLSolution);