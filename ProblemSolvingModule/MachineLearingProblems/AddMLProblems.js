...
const Langauge = 'Machine Learning';
const extraData = null;


class AddMLProblem extends React.Component{
    state = {
        isFocused: false, 
            ...
            if(this.state.descriptionText==='' || this.state.explanationText===''){
                ...
                await fetch(Upload)
                    ...
                    .then(async (blob)=>{   
                        ...
                        var imagesRef = storageRef.child('MLProblems/image'+(descriptionText)+'.jpg');
                        await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                    //console.log(imagesRef);
                    
                        //this.setState({Upload: imagesRef});
                    });  
                        const url =  firebase.storage().ref().child('MLProblems/image'+(descriptionText)+'.jpg')
                        const UploadProb = await url.getDownloadURL();
                       
                // const Captured = firebase.storage().ref().child(Capture).fullPath
                 await this.props.MLproblemStored(name, uid, Langauge, descriptionText, explanationText, UploadProb)
    ...
    handleUpdateButton = async () => {
        ...
        if(des===''){
            ...
            .then(async (blob)=>{   
                var storage = firebase.storage();
                var storageRef = storage.ref();
                var imagesRef = storageRef.child('MLProblems/image'+(des)+'.jpg');
                await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
            });  
                var storageRef = firebase.storage().ref();
                const url =  firebase.storage().ref().child('MLProblems/image'+(des)+'.jpg')
                const UploadProb = await url.getDownloadURL();
            await firebase.database().ref('Probelms/ML/'+this.state.key).update({
                Upload: img,
                descriptionText: des,
                explanationText: exp,
            })
...
const styles = {
  ...
  export default connect(mapStateToProps, actions)(AddMLProblem);