...
const Langauge = 'Android';
const extraData = null;


class AddAndroidtProblem extends React.Component{
    state = {
        isFocused: false, 
                ...
                if(this.state.Upload!==''){
                    await fetch(Upload)
                        ...
                        .then(async (blob)=>{   
                            ...
                            var imagesRef = storageRef.child('AndroidProblems/image'+(descriptionText)+'.jpg');
                            await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                        //console.log(imagesRef);
                        
                            //this.setState({Upload: imagesRef});
                        });  
                            const url =  firebase.storage().ref().child('AndroidProblems/image'+(descriptionText)+'.jpg')
                            const UploadProb = await url.getDownloadURL();
                        
                    // const Captured = firebase.storage().ref().child(Capture).fullPath
                    await this.props.AndroidproblemStored(name, uid, Langauge, descriptionText, explanationText, UploadProb)
                    this.setState({descriptionText:'', explanationText:'', Upload: '', loading: false},() => {
                        this.props.navigation.navigate('androidProb');
                    })
                }
                else {
                    await this.props.AndroidproblemStored1(name, uid, Langauge, descriptionText, explanationText)
                    this.setState({descriptionText:'', explanationText:'', Upload: '', loading: false},() => {
                        this.props.navigation.navigate('androidProb');
                    })
                }
        ...
        if(des===''){
            ...
            if(this.state.img!==''){
                ...
                .then(async (blob)=>{   
                    var storage = firebase.storage();
                    var storageRef = storage.ref();
                    var imagesRef = storageRef.child('AndroidProblems/image'+(des)+'.jpg');
                    await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                });  
                    var storageRef = firebase.storage().ref();
                    const url =  firebase.storage().ref().child('AndroidProblems/image'+(des)+'.jpg')
                    const UploadProb = await url.getDownloadURL();
                await firebase.database().ref('Probelms/Android/'+this.state.key).update({
                    Upload: img,
                    descriptionText: des,
                    explanationText: exp,
                })
...
const styles = {
  ...
  export default connect(mapStateToProps, actions)(AddAndroidtProblem);