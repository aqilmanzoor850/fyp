...
class AndroidSolutions extends React.Component{
    ...
    componentDidMount() {
        ...
        // this.state.solution = [];
            ...
            if(done===null){
                firebase.database().ref('/Solutions/Android')
                .on('value', snapshot => {
    ...
    addReactSolutionHandler() {
        ...
        //  this.state.solution=[];
         //this.isMount=false;
         this.props.navigation.navigate('addAndroidSolution',{key:tId});
    }
    ...
    handleEdit = () => {
        ...
        this.props.navigation.navigate('addAndroidProb', {des:des, exp:exp, img:img, key:tId, states:states});
    }
    ...
    handleDelete = async () => {
        ...
        await firebase.database().ref('Probelms/Android/'+this.state.tId).remove();
        this.props.navigation.navigate('androidProb');
            firebase.database().ref('/Solutions/Android')
            .on('value', snapshot => {
            snapshot.forEach((child)=>{
                if(child.val().key===this.state.tId){
                    firebase.database().ref('/Solutions/Android/' + child.key).remove();
                }
            });
        })
    }
    ...
    handleEditSolution = (sol, upload, id) => {
        ...
        this.props.navigation.navigate('addAndroidSolution', {editSol: sol, editImg: Upload, id: id, states: this.state.states})
    }

    handleDeleteSolution = async (id) => {
         loading=true;
         solution=[];
        await firebase.database().ref('Solutions/Android/'+id).remove();
