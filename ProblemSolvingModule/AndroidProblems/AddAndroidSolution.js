...
class AddAndroidSolution extends React.Component{
            ...
            if(this.state.Upload!==''){
                    await fetch(Upload)
                        ...
                        .then(async (blob)=>{   
                            ...
                            var imagesRef = storageRef.child('AndroidSolutions/image'+(solutionText)+'.jpg');
                            await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                        //console.log(imagesRef);
                        
                            //this.setState({Upload: imagesRef});
                        });  
                            const url =  firebase.storage().ref().child('AndroidSolutions/image'+(solutionText)+'.jpg')
            ...
            if(editImg!==''){
                await fetch(editImg)
                        ...
                        .then(async (blob)=>{   
                            var storage = firebase.storage();
                            var storageRef = storage.ref();
                            var imagesRef = storageRef.child('AndroidSolutions/image'+(editSol)+'.jpg');
                            await imagesRef.put(blob).then((snapshot)=>console.log('Uploaded'));
                        //console.log(imagesRef);
                        
                            //this.setState({Upload: imagesRef});
                        });  
                            const url =  firebase.storage().ref().child('AndroidSolutions/image'+(editSol)+'.jpg')
                            const UploadSol = await url.getDownloadURL();
                await firebase.database().ref('Solutions/Android/'+id).update({
                    UploadSol: UploadSol,
...
const styles = {
  ...
  export default connect(mapStateToProps, actions)(AddAndroidSolution);